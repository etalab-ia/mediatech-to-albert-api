...
DEV_ALBERT_API_URL = "https://albert.api.dev.etalab.gouv.fr"

REPO_ROOT = Path(__file__).parent.parent


def test_dole_e2e_sync_and_retrieval(settings, tmp_path):
    """Full E2E: no results before sync, results after sync."""

    with AlbertClient(DEV_ALBERT_API_URL, settings.albert_api_token) as albert_client:
        ...
        env = {
            **os.environ,
            ...
            "TCHAP_ACCESS_TOKEN": "",
            "ALBERT_API_URL": DEV_ALBERT_API_URL,
        }
        proc = subprocess.run(
        ...
        assert any(EXPECTED_CHUNK_TEXT in result.chunk["content"] for result in results), (
            f"Expected chunk text not found in any of the {len(results)} results"
        )
